...
1. Download the asset ``TW1QuestCreator.exe`` of that release next to the
   running exe as ``TW1QuestCreator.exe.new``.
...
REPO = 'MedievalDev/TW1_DialogAndQuestCreator'
API_LATEST = f'https://api.github.com/repos/{REPO}/releases/latest'
LATEST_PAGE = f'https://github.com/{REPO}/releases/latest'
ASSET = 'TW1QuestCreator.exe'
TIMEOUT = 6
...
def fetch_latest(timeout=TIMEOUT):
    ...
    req = urllib.request.Request(API_LATEST, headers={
        ...
        'User-Agent': f'TW1QuestCreator/{VERSION}'})
...
def start_swap(exe, new, pid=None):
    ...
    fd, path = tempfile.mkstemp(prefix='qf2_update_', suffix='.bat')
